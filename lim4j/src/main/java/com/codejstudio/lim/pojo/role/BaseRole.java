...
public class BaseRole extends GenericElement {
	...
	public boolean setEntity(final Entity entity) throws LIMException {
		...
		if (this.entity != null) {
			...
			flag &= super.removeInnerElementDelegate(this.entity);
			this.entity = null;
		} else {
			this.entity = entity;
			flag &= super.addInnerElementDelegate(entity);
	...
	public boolean removeComment(final Comment... elements) throws LIMException {
		return removeComment((elements == null) ? null : Arrays.asList(elements));
	}

	public boolean removeComment(final Collection<Comment> elementCollection) throws LIMException {
		if (CollectionUtil.checkNullOrEmpty(elementCollection) 
				|| GroupableUtil.checkNullOrEmpty(this.commentGroup)) {
			ElementTrace.log.warn(this.toBaseString() + "fail to removeComment(" 
					+ BaseElement.toBaseString(elementCollection) + ")");
			return false;
		}

		try {
			boolean flag = super.removeInnerElementDelegate(elementCollection) 
					& this.commentGroup.removeGroupElement(elementCollection);
			if (flag) {
				ElementTrace.log.info(this.toBaseString() + ": removeComment(" 
						+ BaseElement.toBaseString(elementCollection) + ")");
			} else {
				ElementTrace.log.warn(this.toBaseString() + "fail to removeComment(" 
						+ BaseElement.toBaseString(elementCollection) + ")");
	...
	private void reloadFromRootElementMap(final Map<String, GenericElement> rootElementMap) 
		...
		if (this.baseEntity != null && this.baseEntity.getId() != null) {
			...
			this.entity = (ge instanceof Entity) ? (Entity) ge : this.entity;
			super.addInnerElementDelegate(this.entity);
		}
	...
	private BaseRole cloneToElement(final BaseRole clonedElement, 
		...
		clonedElement.entity = (this.entity != null) 
				? (Entity) this.entity.cloneElement(clonedElementMap) : clonedElement.entity;
