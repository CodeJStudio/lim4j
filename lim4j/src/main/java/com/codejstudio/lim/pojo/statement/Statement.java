...
public class Statement extends InformationUnit implements IAttributable, IDynamicable, IConditionable {
	...
	public boolean removeConcept(final Concept... concepts) throws LIMException {
		return removeConcept((concepts == null) ? null : Arrays.asList(concepts));
	}

	public boolean removeConcept(final Collection<Concept> conceptCollection) throws LIMException {
		if (CollectionUtil.checkNullOrEmpty(conceptCollection) 
				|| GroupableUtil.checkNullOrEmpty(this.conceptGroup)) {
			ElementTrace.log.warn(this.toBaseString() + "fail to removeConcept(" 
					+ BaseElement.toBaseString(conceptCollection) + ")");
			return false;
		}

		try {
			boolean flag = super.removeInnerElementDelegate(conceptCollection) 
					& this.conceptGroup.removeGroupElement(conceptCollection);
			if (flag) {
				ElementTrace.log.info(this.toBaseString() + ": removeConcept(" 
						+ BaseElement.toBaseString(conceptCollection) + ")");
			} else {
				ElementTrace.log.warn(this.toBaseString() + "fail to removeConcept(" 
						+ BaseElement.toBaseString(conceptCollection) + ")");
	...
	@Override
	public String substituteDynamicIdentifier(final String[] inputArray) {
