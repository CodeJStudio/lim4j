...
public class Argument extends InformationUnit implements IAttributable, IConditionable, IValuable {
	...
	public boolean setValidity(final Validity validity) throws LIMException {
		...
		if (v != null) {
			...
			flag &= super.removeInnerElementDelegate(v) 
	...
	private boolean setCausalityRelation(final CausalityRelation causalityRelation) throws LIMException {
		if (Objects.equals(this.causalityRelation, causalityRelation)) {
			return true;
		}
		...
		if (this.causalityRelation != null) {
			...
			flag &= super.removeInnerElementDelegate(this.causalityRelation);
			this.causalityRelation = null;
		}
		if (causalityRelation != null) {
			this.causalityRelation = causalityRelation;
			flag &= super.addInnerElementDelegate(causalityRelation);
	...
	private String assembleArgumentDescription(final JudgedStatementGroup evidenceGroup, 
			final JudgedStatementGroup conclusionGroup) {
	...
	public boolean removeEvidence(final JudgedStatement... evidences) throws LIMException {
		return removeEvidence((evidences == null) ? null : Arrays.asList(evidences));
	}

	public boolean removeEvidence(final Collection<JudgedStatement> evidenceCollection) throws LIMException {
		Attribute at;
		if (CollectionUtil.checkNullOrEmpty(evidenceCollection) 
				|| (at = getAttributeByKey(EVIDENCE)) == null) {
				ElementTrace.log.warn(this.toBaseString() + "fail to removeEvidence(" 
						+ BaseElement.toBaseString(evidenceCollection) + ")");
			return false;
		}

		try {
			boolean flag = super.removeInnerElementDelegate(evidenceCollection) 
					& removeJudgedStatement(evidenceCollection) 
					& at.removeValue(evidenceCollection);
			if (flag) {
				ElementTrace.log.info(this.toBaseString() + ": removeEvidence(" 
						+ BaseElement.toBaseString(evidenceCollection) + ")");
			} else {
				ElementTrace.log.warn(this.toBaseString() + "fail to removeEvidence(" 
						+ BaseElement.toBaseString(evidenceCollection) + ")");
	...
	public boolean removeConclusion(final JudgedStatement... conclusions) throws LIMException {
		return removeConclusion((conclusions == null) ? null : Arrays.asList(conclusions));
	}

	public boolean removeConclusion(final Collection<JudgedStatement> conclusionCollection) throws LIMException {
		Attribute at;
		if (CollectionUtil.checkNullOrEmpty(conclusionCollection) 
				|| (at = getAttributeByKey(CONCLUSION)) == null) {
			ElementTrace.log.warn(this.toBaseString() + "fail to removeConclusion(" 
					+ BaseElement.toBaseString(conclusionCollection) + ")");
			return false;
		}

		try {
			boolean flag = super.removeInnerElementDelegate(conclusionCollection) 
					& removeJudgedStatement(conclusionCollection) 
					& at.removeValue(conclusionCollection);
			if (flag) {
				ElementTrace.log.info(this.toBaseString() + ": removeConclusion(" 
						+ BaseElement.toBaseString(conclusionCollection) + ")");
			} else {
				ElementTrace.log.warn(this.toBaseString() + "fail to removeConclusion(" 
						+ BaseElement.toBaseString(conclusionCollection) + ")");
	...
	public boolean removeSubArgument(final Argument... arguments) throws LIMException {
		return removeSubArgument((arguments == null) ? null : Arrays.asList(arguments));
	}

	public boolean removeSubArgument(final Collection<Argument> argumentCollection) throws LIMException {
		if (CollectionUtil.checkNullOrEmpty(argumentCollection) 
				|| GroupableUtil.checkNullOrEmpty(this.subArgumentGroup)) {
			ElementTrace.log.warn(this.toBaseString() + "fail to removeSubArgument(" 
					+ BaseElement.toBaseString(argumentCollection) + ")");
			return false;
		}

		try {
			boolean flag = super.removeInnerElementDelegate(argumentCollection) 
					& this.subArgumentGroup.removeGroupElement(argumentCollection);
			if (flag) {
				ElementTrace.log.info(this.toBaseString() + ": removeSubArgument(" 
						+ BaseElement.toBaseString(argumentCollection) + ")");
			} else {
				ElementTrace.log.warn(this.toBaseString() + "fail to removeSubArgument(" 
						+ BaseElement.toBaseString(argumentCollection) + ")");
	...
	public boolean removeJudgedStatement(final JudgedStatement... statements) throws LIMException {
		return removeJudgedStatement((statements == null) ? null : Arrays.asList(statements));
	}

	public boolean removeJudgedStatement(final Collection<JudgedStatement> statementCollection) throws LIMException {
		if (CollectionUtil.checkNullOrEmpty(statementCollection) 
				|| GroupableUtil.checkNullOrEmpty(this.judgedStatementGroup)) {
			ElementTrace.log.warn(this.toBaseString() + "fail to removeJudgedStatement(" 
					+ BaseElement.toBaseString(statementCollection) + ")");
			return false;
		}

		try {
			boolean flag = super.removeInnerElementDelegate(statementCollection) 
					& this.judgedStatementGroup.removeGroupElement(statementCollection);
			if (flag) {
				ElementTrace.log.info(this.toBaseString() + ": removeJudgedStatement(" 
						+ BaseElement.toBaseString(statementCollection) + ")");
			} else {
				ElementTrace.log.warn(this.toBaseString() + "fail to removeJudgedStatement(" 
						+ BaseElement.toBaseString(statementCollection) + ")");
	...
	@Override
	public boolean removeCondition(final Condition... conditions) throws LIMException {
		return removeCondition((conditions == null) ? null : Arrays.asList(conditions));
	}

	@Override
	public boolean removeCondition(final Collection<Condition> conditionCollection) throws LIMException {
		if (CollectionUtil.checkNullOrEmpty(conditionCollection) 
				|| GroupableUtil.checkNullOrEmpty(this.conditionGroup)) {
			ElementTrace.log.warn(this.toBaseString() + "fail to removeCondition(" 
					+ BaseElement.toBaseString(conditionCollection) + ")");
			return false;
		}

		try {
			boolean flag = super.removeInnerElementDelegate(conditionCollection) 
					& this.conditionGroup.removeGroupElement(conditionCollection);
			if (flag) {
				ElementTrace.log.info(this.toBaseString() + ": removeCondition(" 
						+ BaseElement.toBaseString(conditionCollection) + ")");
			} else {
				ElementTrace.log.warn(this.toBaseString() + "fail to removeCondition(" 
						+ BaseElement.toBaseString(conditionCollection) + ")");
	...
	public boolean removeAttributeByKey(final String key) throws LIMException {
		return removeAttribute(getAttributeByKey(key));
	}
	...
	public boolean removeAttribute(final String key, final GenericElement value) throws LIMException {
		...
		return (at == null) ? false 
				: ((at.getCompatible() == null || !at.getCompatible()) 
						? removeAttribute(at) : at.removeValue(value));
	}
	...
	@Override
	public boolean removeAttribute(final Collection<Attribute> attributeCollection) throws LIMException {
		if (CollectionUtil.checkNullOrEmpty(attributeCollection) 
				|| GroupableUtil.checkNullOrEmpty(this.attributeGroup)) {
		...
		try {
			...
			for (Attribute at : attributeCollection) {
				if (at != null) {
					flag &= super.removeInnerElementDelegate(at) 
							& this.attributeGroup.removeAttribute(at) 
							& super.removeInnerElementDelegate(
									new DummyRelation(AffiliationRelation.class, this, at));
				}
			}
			if (flag) {
	...
	private void reloadFromRootElementMap(final Map<String, GenericElement> rootElementMap) 
		...
		if (this.baseCausalityRelation != null && this.baseCausalityRelation.getId() != null) {
			...
			this.causalityRelation = (ge instanceof CausalityRelation) 
					? (CausalityRelation) ge : this.causalityRelation;
			super.addInnerElementDelegate(this.causalityRelation);
		}
	...
	private Argument cloneToElement(final Argument clonedElement, 
		...
		clonedElement.causalityRelation = (this.causalityRelation != null) 
				? (CausalityRelation) this.causalityRelation.cloneElement(clonedElementMap) 
						: clonedElement.causalityRelation;
