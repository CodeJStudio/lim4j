...
public class Entity extends GenericElement {
	...
	public boolean removeProposedElement(final OwnableInformationElement... elements) throws LIMException {
		return removeProposedElement((elements != null) ? Arrays.asList(elements) : null);
	}

	public boolean removeProposedElement(final Collection<OwnableInformationElement> elementCollection) 
			throws LIMException {
		if (CollectionUtil.checkNullOrEmpty(elementCollection) 
				|| GroupableUtil.checkNullOrEmpty(this.proposedElementGroup)) {
			ElementTrace.log.warn(this.toBaseString() + "fail to removeProposedElement(" 
					+ BaseElement.toBaseString(elementCollection) + ")");
			return false;
		}
		...
		try {
			boolean flag = super.removeInnerElementDelegate(elementCollection) 
					& this.proposedElementGroup.removeGroupElement(elementCollection);
			if (flag) {
				ElementTrace.log.info(this.toBaseString() + ": removeProposedElement(" 
						+ BaseElement.toBaseString(elementCollection) + ")");
			} else {
				ElementTrace.log.warn(this.toBaseString() + "fail to removeProposedElement(" 
						+ BaseElement.toBaseString(elementCollection) + ")");
	...
	public boolean removeObservedElement(final OwnableInformationElement... elements) throws LIMException {
		return removeObservedElement((elements != null) ? Arrays.asList(elements) : null);
	}

	public boolean removeObservedElement(final Collection<OwnableInformationElement> elementCollection) 
			throws LIMException {
		if (CollectionUtil.checkNullOrEmpty(elementCollection) 
				|| GroupableUtil.checkNullOrEmpty(this.observedElementGroup)) {
			ElementTrace.log.warn(this.toBaseString() + "fail to removeObservedElement(" 
					+ BaseElement.toBaseString(elementCollection) + ")");
			return false;
		}
		...
		try {
			boolean flag = super.removeInnerElementDelegate(elementCollection) 
					& this.observedElementGroup.removeGroupElement(elementCollection);
			if (flag) {
				ElementTrace.log.info(this.toBaseString() + ": removeObservedElement(" 
						+ BaseElement.toBaseString(elementCollection) + ")");
			} else {
				ElementTrace.log.warn(this.toBaseString() + "fail to removeObservedElement(" 
						+ BaseElement.toBaseString(elementCollection) + ")");
