...
public abstract class AbstractRelationableInformationElement extends OwnableInformationElement 
	...
	@Override
	public boolean removeRelation(final BaseRelation... relations) throws LIMException {
		return removeRelation((relations == null) ? null : Arrays.asList(relations));
	}

	@Override
	public boolean removeRelation(final Collection<BaseRelation> relationCollection) throws LIMException {
		if (CollectionUtil.checkNullOrEmpty(relationCollection) 
				|| GroupableUtil.checkNullOrEmpty(this.relationGroup)) {
			ElementTrace.log.warn(this.toBaseString() + "fail to removeRelation(" 
					+ BaseElement.toBaseString(relationCollection) + ")");
			return false;
		}

		try {
			boolean flag = super.removeInnerElementDelegate(relationCollection) 
					& this.relationGroup.removeGroupElement(relationCollection);
			if (flag) {
				ElementTrace.log.info(this.toBaseString() + ": removeRelation(" 
						+ BaseElement.toBaseString(relationCollection) + ")");
			} else {
				ElementTrace.log.warn(this.toBaseString() + "fail to removeRelation(" 
						+ BaseElement.toBaseString(relationCollection) + ")");
