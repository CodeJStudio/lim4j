...
public class Concept extends InformationUnit implements IAttributable, IConditionable {
	...
	public boolean removeSubConcept(final Concept... concepts) throws LIMException {
		return removeSubConcept((concepts == null) ? null : Arrays.asList(concepts));
	}

	public boolean removeSubConcept(final Collection<Concept> conceptCollection) throws LIMException {
		if (CollectionUtil.checkNullOrEmpty(conceptCollection) 
				|| GroupableUtil.checkNullOrEmpty(this.subConceptGroup)) {
			ElementTrace.log.warn(this.toBaseString() + "fail to removeSubConcept(" 
		...
		try {
			boolean flag = super.removeInnerElementDelegate(conceptCollection) 
					& this.subConceptGroup.removeGroupElement(conceptCollection);
			if (flag) {
				ElementTrace.log.info(this.toBaseString() + ": removeSubConcept(" 
						+ BaseElement.toBaseString(conceptCollection) + ")");
			} else {
				ElementTrace.log.warn(this.toBaseString() + "fail to removeSubConcept(" 
						+ BaseElement.toBaseString(conceptCollection) + ")");
