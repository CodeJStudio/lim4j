...
public class ConceptGroup extends Concept implements IGroupable<Concept> {
	...
	private List<Concept> innerGroupList;
	...
	@Override
	public List<Concept> getInnerGroupList() {
	...
	@Override
	public boolean removeGroupElement(final Concept... groupElements) throws LIMException {
		return removeGroupElement((groupElements == null) ? null : Arrays.asList(groupElements));
	}

	@Override
	public boolean removeGroupElement(final Collection<? extends Concept> groupElementCollection) 
			throws LIMException {
		if (CollectionUtil.checkNullOrEmpty(groupElementCollection) 
	...
	@Override
	public boolean replaceGroupElement(final Concept replacee, final Concept replacer) throws LIMException {
	...
	private void reloadFromRootElementMap(final Map<String, GenericElement> rootElementMap) 
		...
		for (BaseElement be : vc) {
			...
			this.innerGroupList.add((Concept) ge);
		}
		destroyInnerGroupList();
	}
	...
	private ConceptGroup cloneToElement(final ConceptGroup clonedElement, 
		...
		if (!CollectionUtil.checkNullOrEmpty(this.innerGroupList)) {
			...
			for (Concept element : this.innerGroupList) {
