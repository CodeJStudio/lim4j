...
public class Attribute extends InformationUnit {
	...
	protected GenericElement value;
	...
	private void destroyValueGroup() throws LIMException {
		if (this.valueGroup != null && this.valueGroup.size() == 0) {
			this.baseValueGroup = null;
			super.removeInnerElementDelegate(this.valueGroup);
	...
	private boolean setValue(final GenericElement value) throws LIMException {
		...
		if (this.value != null) {
			this.baseValue = null;
			flag &= super.removeInnerElementDelegate(this.value);
			this.value = null;
		}
		if (value != null) {
			this.value = value;
			flag &= super.addInnerElementDelegate(value);
	...
	public boolean addValue(final Collection<? extends GenericElement> valueCollection) throws LIMException {
		if (CollectionUtil.checkNullOrEmpty(valueCollection) 
				|| CollectionUtil.onlyContains(valueCollection, this.value) 
				|| (this.valueGroup != null 
	...
	public boolean removeValue(final GenericElement... values) throws LIMException {
		return removeValue((values != null) ? Arrays.asList(values) : null);
	}

	public boolean removeValue(final Collection<? extends GenericElement> valueCollection) throws LIMException {
		if (CollectionUtil.checkNullOrEmpty(valueCollection) 
				|| GroupableUtil.checkNullOrEmpty(this.valueGroup)) {
			ElementTrace.log.warn(this.toBaseString() + "fail to removeValue(" 
					+ BaseElement.toBaseString(valueCollection) + ")");
			return false;
		}
		...
		try {
			...
			if (this.compatible == null || !this.compatible) {
				if (this.value != null && this.value.equals(valueCollection.iterator().next())) {
					flag &= setValue(null);
				}
			} else {
				flag &= super.removeInnerElementDelegate(valueCollection);
			}
			flag &= this.valueGroup.removeGroupElement(valueCollection);
			...
			if (flag) {
				ElementTrace.log.info(this.toBaseString() + ": removeValue(" 
						+ BaseElement.toBaseString(valueCollection) + ")");
			} else {
				ElementTrace.log.warn(this.toBaseString() + "fail to removeValue(" 
						+ BaseElement.toBaseString(valueCollection) + ")");
	...
	private void reloadFromRootElementMap(final Map<String, GenericElement> rootElementMap) 
		...
		if (this.baseValue != null && this.baseValue.getId() != null) {
			...
			this.value = (ge != null) ? ge : this.value;
			super.addInnerElementDelegate(this.value);
		}
	...
	private Attribute cloneToElement(final Attribute clonedElement, 
		...
		clonedElement.value = (this.value != null) 
				? (GenericElement) this.value.cloneElement(clonedElementMap) : clonedElement.value;
