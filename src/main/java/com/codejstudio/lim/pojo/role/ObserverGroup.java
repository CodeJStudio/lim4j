 ...
 * @since   lim4j_v1.0.0
 */
public class ObserverGroup extends Observer implements IGroupable<Observer> {

	/* variables: collections, maps, sub-groups */
	
	private Collection<Observer> innerGroupCollection;
	 ...
	 */
	public ObserverGroup() throws LIMException {
		super();
	}

	public ObserverGroup(boolean ifInitId) throws LIMException {
		super(ifInitId, true);
	}

	public ObserverGroup(boolean ifInitId, Observer... observers) throws LIMException {
		super(ifInitId, true);
		addGroupElement(observers);
	}


	public ObserverGroup(Observer... observers) throws LIMException {
		this(true, observers);
	}
	...
	@Override
	public Collection<Observer> getInnerGroupCollection() {
	...
	@Override
	public boolean containGroupElement(Observer element) throws LIMException{
		return (this.innerGroupCollection != null) 
				? this.innerGroupCollection.contains(element) : false;
	}

	@Override
	public boolean addGroupElement(Observer... elements) throws LIMException {
		return addGroupElement(CollectionUtil.generateCollection(elements));
	}

	@Override
	public boolean addGroupElement(Collection<Observer> elements) throws LIMException {
	...
	@Override
	public boolean removeGroupElement(Observer... elements) throws LIMException {
		return removeGroupElement(CollectionUtil.generateCollection(elements));
	}

	@Override
	public boolean removeGroupElement(Collection<Observer> elements) throws LIMException {
		if(CollectionUtil.checkNullOrEmpty(elements) 
	...
	private void reloadFromRootElementMap(Map<String, AbstractElement> rootElementMap) throws LIMException {
		...
		for (BaseElement be : c) {
			if(be != null && be.getId() != null) {
				...
				if(ae != null && ae instanceof Observer) {
					Observer element = (Observer) ae;
					this.innerGroupCollection.add(element);
				}
			}
		}
	...
	@Override
	public ObserverGroup cloneElement() throws LIMException {
		ObserverGroup cloneElement = (ObserverGroup) super.cloneElement();
		
		if(!CollectionUtil.checkNullOrEmpty(this.innerGroupCollection)) {
			cloneElement.initInnerGroupCollection();
			for (Observer element : this.innerGroupCollection) {
				if(element != null && !this.absoluteEquals(element)) {
					cloneElement.innerGroupCollection.add((Observer) element.cloneElement());
				}
			}
		}
